  ...
  try {
    ...
    if (!(await Bun.file(fontPath).exists())) {
      ...
      const response = await fetch(
        `https://fonts.googleapis.com/css2?family=${fontQuery}`,
      )
    ...
    switch (element.type) {
      ...
      case 'svg':
        try {
          ...
          const modifiedSvgString = replaceSvgColors(
            svgString,
            element.colorsReplace,
          )

          // Add width and height to SVG string if not present
          let finalSvgString = modifiedSvgString
          if (
            !finalSvgString.includes('width=') ||
            !finalSvgString.includes('height=')
          ) {
